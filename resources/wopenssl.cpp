...
SF_SSL_READ(int _fd, SSL* _ssl, char* _buffer , int _sizebuf)
{
    ...
    if (_rc <= 0)
    {
        std::cerr   << "%log wopenssl: select timeout or trouble"
                    << std::endl;
    }
    ...
    if (_bytes > 0)
    {
        std::cout   << "%log wopenssl: SSL read data ok"
                    << std::endl;
    } else {
        std::cerr   << "%log wopenssl: SSL_read have problem"
                    << std::endl;
    }
...
SF_SSL_WRITE(int _fd, SSL* _ssl, char* _buffer , int _sizebuf)
{
    ...
    if (_bytes > 0)
    {
        std::cout   << "%log wopenssl: ssl write data ok"
                    << std::endl;
    } else {
        std::cerr   << "%log wopenssl: SSL_write have problem"
                    << std::endl;
    }
